...
from reconstructed_minicode.config import (
...
def try_handle_local_command(user_input: str, tools=None, agent_registry=None) -> str | None:
    ...
    if user_input == "/state":
        try:
            from reconstructed_minicode.agent.state import handle_state_command
    ...
    if user_input == "/context":
        ...
        try:
            from reconstructed_minicode.agent.context import load_context_state
    ...
    if user_input == "/status":
        ...
        from reconstructed_minicode.model.registry import detect_provider
    ...
    if user_input == "/model":
        try:
            runtime = load_runtime_config()
            from reconstructed_minicode.model.registry import format_model_status
    ...
    if user_input.startswith("/model "):
        ...
        if not arg:
            from reconstructed_minicode.model.registry import format_model_list
        ...
        from reconstructed_minicode.model.registry import Provider, format_model_list
        for p in Provider:
    ...
    if user_input == "/user" or user_input.startswith("/user "):
        from reconstructed_minicode.utils.user_profile import handle_user_command
