...
from reconstructed_minicode.tui.input_parser import KeyEvent, ParsedInputEvent, TextEvent, WheelEvent
from reconstructed_minicode.tui.navigation import (
...
def _handle_normal_mode_return(
    ...
    if visible_commands and 0 <= state.selected_slash_index < len(visible_commands):
        ...
        if state.input.strip() != usage.strip():
            # Input is a prefix — fill in the full command and wait for next Enter
            state.input = usage
            state.cursor_offset = len(state.input)
            state.selected_slash_index = 0
            rerender()
            return
        # Input already exactly matches — fall through and submit
