...
from reconstructed_minicode.model.retry import RETRYABLE_STATUS, calculate_backoff
from reconstructed_minicode.utils.cost_tracker import calculate_cost
from reconstructed_minicode.agent.state import Store, AppState, add_cost, record_api_error, update_context_usage
    ...
    def next(
        ...
        if status >= 400:
            ...
            try:
                ...
                error_msg = error_data.get("error", {}).get("message", f"API error: {status}")
            except Exception:
                error_msg = f"API error {status}: {error_body[:200]}"
            raise RuntimeError(error_msg)
