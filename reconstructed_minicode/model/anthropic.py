...
from reconstructed_minicode.model.retry import (
    RETRYABLE_STATUS,
    calculate_backoff,
)
from reconstructed_minicode.types import AgentStep, StepDiagnostics
...
class AnthropicModelAdapter:
    ...
    def next(self, messages: list[dict[str, Any]], on_stream_chunk: Callable[[str], None] | None = None, store: Store[AppState] | None = None) -> AgentStep:
        ...
        request_body["max_tokens"] = self.runtime.get("maxOutputTokens") or 16384
        if on_stream_chunk:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                **{"x-api-key": self.runtime.get("apiKey") or self.runtime.get("authToken", "")},
            },
            method="POST",
        )
        ...
        if not on_stream_chunk:
            ...
            if store:
                ...
                from reconstructed_minicode.utils.cost_tracker import calculate_cost
        ...
        # STREAMING PARSER
        # Check HTTP status before iterating — an HTTPError response body is plain
        # JSON, not SSE, so the loop below would silently skip it and return empty.
        status = getattr(response, "status", getattr(response, "code", 200))
        if status >= 400:
            data = _read_json_body(response)
            raise RuntimeError(_extract_error_message(data, status))
