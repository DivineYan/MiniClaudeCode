...
def load_runtime_config(cwd: str | Path | None = None) -> dict[str, Any]:
    ...
    # Apply settings.json env section to os.environ so provider detection
    # (which reads os.environ directly) can see these values.
    for k, v in effective.get("env", {}).items():
...
def format_config_diagnostic(cwd: str | Path | None = None) -> str:
    ...
    try:
        ...
        # Show provider info
        from reconstructed_minicode.model.registry import detect_provider, Provider
